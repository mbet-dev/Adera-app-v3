...
-- ============================================================
-- CUSTOM TYPES (created once, with safe idempotency)
-- ============================================================
...
-- ============================================================
-- USERS TABLE (consolidated — replaces both legacy "users" and "profiles")
-- ============================================================
-- This is the single source of truth for all user data.
-- The auth trigger populates this row when a new Supabase Auth user signs up.
...
-- Users: can read/update own data; admin can see all
...
-- Shops: owners full access, public can read active shops
...
-- Products: shop owners manage, public reads available products from active shops
...
-- Parcel events: same visibility as parent parcel
...
-- Orders: customer and shop owner access
...
BEGIN
  ...
  user_role_val := coalesce(nullif((meta->>'role')::text,''), default_role);
  user_full_name := coalesce(
    nullif(meta->>'full_name',''),
    nullif(new.raw_user_meta_data->>'full_name',''),
    nullif(new.email,'')
  );
  user_first_name := coalesce(nullif(meta->>'first_name',''), '');
  user_last_name := coalesce(nullif(meta->>'last_name',''), '');
  user_phone := coalesce(nullif(new.phone,''), nullif(meta->>'phone',''));
  user_lang := coalesce(nullif(meta->>'language',''), 'en');
  avatar := coalesce(nullif(meta->>'avatar_url',''), nullif(meta->>'avatar',''));

  -- Handle existing user (update missing fields only)
  IF EXISTS(SELECT 1 FROM public.users WHERE id = new.id) THEN
    UPDATE public.users SET
      email = coalesce(public.users.email, new.email),
      full_name = coalesce(nullif(public.users.full_name, ''), user_full_name),
      first_name = coalesce(nullif(public.users.first_name, ''), nullif(user_first_name, '')),
      last_name = coalesce(nullif(public.users.last_name, ''), nullif(user_last_name, '')),
      phone = coalesce(nullif(public.users.phone, ''), user_phone),
      role = coalesce(public.users.role, user_role_val::user_role),
      language = coalesce(nullif(public.users.language, ''), user_lang),
      avatar_url = coalesce(nullif(public.users.avatar_url, ''), avatar),
      email_confirmed = coalesce(public.users.email_confirmed, (new.email_confirmed_at IS NOT NULL)),
      updated_at = NOW()
    WHERE id = new.id;
    RETURN new;
  END IF;
  ...
  INSERT INTO public.users (
    ...
    language, avatar_url, email_confirmed, created_at
  ) VALUES (
    new.id, new.email, user_full_name,
    nullif(user_first_name, ''), nullif(user_last_name, ''),
    user_phone, user_role_val::user_role,
    user_lang, avatar,
